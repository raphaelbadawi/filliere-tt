...
export default function PostSnippet({index, post}: {index: number, post: Post}) {
    return (
        <div key={index} className="mt-4 mx-autos px-10 py-6 rounded bg-gradient-to-r from-primary to-black ">
            ...
            <div className="mt-3">
                <a rel="noopener noreferrer" href={`/news/${post.attributes.slug}`} className="text-3xl font-bold hover:underline">{post.attributes.title}</a>
            ...
            <div className="flex items-center justify-between mt-4">
                <a rel="noopener noreferrer" href={`/news/${post.attributes.slug}`} className="hover:underline text-accent">Voir plus</a>
