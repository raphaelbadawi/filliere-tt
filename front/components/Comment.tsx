...
export default function CommentComponent({comment}: {comment: Comment}) {
    const commentContent = parseLinks(comment.content);
    return (
        <div role="comment" className="p-2 sm:p-4 bg-white text-black rounded">
            ...
            <p className="text-right text-sm">par <span className="text-base font-semibold text-accent">{comment.author}</span> le <span className="font-semibold text-primary"><Date dateString={comment.createdAt}></Date></span></p>
        </div>
    )
}